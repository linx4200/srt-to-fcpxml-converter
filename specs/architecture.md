...
FCPXML 会额外输出一个位于字幕下方的矩形生成器，作为整条序列共享的字幕背景框；背景框使用字幕样式中的背景色、透明度和圆角参数，并根据 Target Video Orientation 选择不同的矩形坐标。横屏字幕标题还会写入 Custom title 的内部文字位置参数，以匹配 Final Cut Pro 手动调整后的底部字幕高度；竖屏字幕标题保留现有外层 `adjust-transform` 位置，不写入该内部位置参数。
...
- 导出的 FCPXML 只包含字幕标题片段和字幕背景框生成器，不包含真实视频、音频或完整剪辑工程结构。
